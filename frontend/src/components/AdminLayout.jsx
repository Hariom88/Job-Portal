...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-[var(--bg-main)] font-['Outfit'] relative overflow-x-hidden">
      {/* MOBILE OVERLAY */}
      <AnimatePresence>
        {isOpen && (
          <motion.div 
      ...
      <motion.aside 
        ...
        className={`
          fixed lg:sticky top-0 left-0 h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col z-50
      ...
      >
        <div className="p-6 lg:p-8 pb-12 flex items-center justify-between lg:justify-start gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl lg:text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Prime<span className="text-blue-600">Admin</span></span>
          </div>
          <button onClick={() => setIsOpen(false)} className="lg:hidden p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white">✕</button>
        </div>
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        {/* TOP NAVBAR */}
        <header className="h-20 bg-[var(--nav-bg)] backdrop-blur-md border-b border-slate-100 dark:border-slate-800 px-4 lg:px-10 flex items-center justify-between sticky top-0 z-30">
           ...
           <div className="flex gap-2 lg:gap-4 items-center">
              ...
              <button 
                onClick={toggleTheme}
                className="w-9 h-9 lg:w-10 lg:h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-colors"
              >
                {isDarkMode ? '☀️' : '🌙'}
              </button>
              <Link 
                to="/admin/settings"
                className="w-9 h-9 lg:w-10 lg:h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-colors no-underline"
              >
                ⚙️
              </Link>
           </div>
